...
@Component({
  selector: 'wn-book-graph',
  templateUrl: './book-graph.component.html',
  styleUrls: ['./book-graph.component.css']
})
export class BookGraphComponent implements OnInit {
  ...
  setup() {
    ...
    const svg = d3.select(this.bookGraph.nativeElement)
      .append('svg')
...
//  - links are always source < target; edge directions are set by 'left' and 'right'.
    ...
    this._edgeService.getBookEdges(this.bookId).subscribe((edges) => {
      this._chapterService.getBookChapters(this.bookId).subscribe(chapters => {
        ...
        let links = graph.edges.map(edge => {
          const sourceId = edge.source
          const targetId = edge.target
          if (edge.source === '59d763fa29704a0012f24940') {
            console.log(edge)
          }

          edge.source = nodes.find(node => node._id === sourceId)
          edge.target = nodes.find((node) => node._id === targetId)
          edge.left = false
          edge.right = true
...
// update graph (called when needed)
        ...
        function spliceLinksForNode(node) {
          const toSplice = links.filter(function (l) {
